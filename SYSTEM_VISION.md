...
| D74 | **El eje de referencia de la estimación propia (D43/D52/D56) ya no es siempre el corneal en bruto (K1/K2): cuando la calculadora da una fila SIN corrección tórica (cilindro 0), su `ejeResidual` manda sobre el eje de K1/K2.** El criterio del cilindro («la última opción tórica que comparte el eje curvo») no cambia; cambia solo CONTRA QUÉ eje se compara. Si no hay fila sin corrección (EVO y Barrett a veces solo dan una fila tórica cada una), se sigue usando el eje corneal de siempre, sin cambio de comportamiento | Fallo real encontrado por el dueño con un PDF real (CV-2026-0141, 10/09/2026): córnea de solo 0.30 D (K1 44.76@2° K2 45.06@92°) con un SIA de 0.25 D — casi tan grande como el propio astigmatismo corneal. El eje corneal en bruto (K2, 92°) no reflejaba el astigmatismo NETO real (córnea + SIA ya combinados), así que el criterio elegía un cilindro de 1.25 D para Kane y 0.9 D para EVO cuando ninguna de las dos calculadoras había destacado esa opción — y Barrett, con su propio algoritmo, había elegido Non Toric (cilindro 0) para el mismo ojo. La fila sin corrección que cada calculadora ya devuelve trae el astigmatismo neto correcto —cornea y SIA ya combinados por la propia web—, así que no hace falta que este programa repita esa combinación vectorial: se usa ese eje como referencia cuando existe. Verificado con los datos reales del caso a través de las funciones de producción (`ejeCurvoDe`, `estimarLenteRecomendada`): las tres calculadoras coinciden ahora en cilindro 0, igual que Barrett ya elegía por su cuenta. Dos tests nuevos en `recomendacion.test.ts` reproducen el caso real exacto |
