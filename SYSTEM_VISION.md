...
| D51 | **Se añade `sugerirOpcion`: aplica el criterio EXPLÍCITO del dueño del proyecto para leer una escalera de potencias, como una sugerencia separada y con motivo escrito — no como una recomendación del programa, y D14 sigue sin tocarse** | Petición del dueño del proyecto (25/08/2026): quiere que la aplicación marque, dentro de cada tabla de opciones, cuál elegiría él siguiendo tres reglas suyas, mecánicas y sin ambigüedad clínica que decidir: en una lente Envista, el residual NEGATIVO más cercano a cero; en una Lux, el POSITIVO más cercano a cero; en una tabla tórica, el mayor cilindro cuyo eje residual no salta ~90° respecto al de la potencia anterior —si salta, la potencia anterior es la buena—. Se le hizo pushback explícito primero, citando D14 («la primera versión no da una recomendación clínica propia») y D37, que ya rechazó marcar una opción del catálogo como «la buena» por ser exactamente esa misma recomendación; el dueño del proyecto confirmó que lo quiere igualmente, como una SUGERENCIA visible, nunca vinculante. La implementación (`packages/domain/src/comparacion/sugerencia-cirujano.ts`) se mantiene deliberadamente separada de `OpcionLente.recomendada` (que es lo que la propia web destaca) — `comparar.ts` documenta, de un fallo real anterior, exactamente el peligro de confundir las dos: una opción elegida por el programa no puede parecer una opción que la calculadora señaló. En pantalla sale con su propio color y su motivo escrito («Según tu criterio: …»), nunca como si lo dijera Kane/EVO/Barrett. La regla del cilindro no necesita ningún eje de referencia externo —ni el astigmatismo neto que publica la calculadora, ni el K1/K2 del informe—: compara cada opción con la anterior en la misma tabla, así que funciona igual aunque una calculadora transponga sus ejes (EVO lo hace), porque la transposición es constante en toda la tabla. Qué familia es una lente (Envista/Lux) se decide por el nombre del modelo elegido en el caso, no por el catálogo ni por cada opción por separado, porque este producto ya compara siempre UNA sola lente elegida entre las tres calculadoras |
