...
| D69 | **Constante A conocida por lente, para Barrett (amplía D33).** Barrett no tiene desplegable de lentes propio —a diferencia de EVO y Kane, que resuelven su propia constante en su propia web—, así que elegir una lente que el informe no trae dejaba siempre la constante vacía para Barrett. Cinco modelos de Bausch & Lomb (B&L Aspire, MX60ET/PT, Envy, LuxSmart, LuxLife) llevan ahora, en el catálogo propio de la app (`SelectorLente.tsx`), una constante A general dada expresamente por el dueño del proyecto. Se aplica SOLO si el informe no trae su propia tabla de lentes para ese modelo —si la trae, esa constante manda siempre, por ser específica de ese paciente— y NUNCA pisa una constante escrita a mano. El propio dueño confirmó que son valores generales del fabricante, **no verificados específicamente para la fórmula de Barrett**: por eso se guardan con procedencia `DERIVADO` y piden comprobación humana antes de calcular, igual que cualquier otro dato que aporta el programa sin que nadie lo haya revisado | Petición expresa del dueño del proyecto (04/09/2026), con los cinco pares modelo/constante dados directamente por él. Antes de implementarlo, Claude preguntó explícitamente si eran constantes confirmadas para Barrett en concreto o un valor general — el dueño confirmó que es el general, no diferenciado por fórmula, y esa distinción es la que decide si el dato pide revisión humana o no |
