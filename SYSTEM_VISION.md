...
| D38 | **Barrett y Kane usan, cada uno, SU PROPIA constante A del catálogo para la misma lente; EVO usa la suya, que rellena sola**  | Petición del dueño del proyecto (23/08/2026), tras comprobar en vivo que EVO, Barrett y Kane publican constantes distintas para la misma lente (enVista ENVY: 119.24 / 119.28 / 119.33). Antes había un único campo `CONSTANTE_A` por ojo, compartido por las tres — no bastaba. Ahora: si el catálogo trae la constante de esa calculadora para la lente elegida, Barrett y Kane la usan (`sustituirConstanteDelCatalogo` en `orquestador.ts`); EVO queda fuera a propósito y no se toca, porque reconoce el modelo por su cuenta y rellena la suya, que es mejor que cualquier número que pudiéramos darle (comprobado: «B&L Envy» → 119.24 sin mandarle nada). El campo compartido del ojo sigue existiendo como valor por defecto cuando la lente no está en el catálogo. Concreta D33 y D34 |
