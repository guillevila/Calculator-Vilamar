...
| D76 | **«Calcular con las dos lentes» — el camino corto de D55, de un solo gesto.** Antes de este cambio, comparar dos lentes exigía cinco pasos manuales: calcular, generar el PDF, volver a los datos, activar la lente aparcada (D55), calcular otra vez, generar el segundo PDF. `ServicioCasos.calcularConDosLentes()` hace la misma secuencia sola: reutiliza `calcular()`, `generarPdf()` e `intercambiarLentes()` tal cual, en ese orden — cero lógica nueva de constantes. Un botón nuevo, «Calcular con las dos lentes («X» y «Y»)» en la pantalla de cálculo, solo aparece con una lente alternativa ya aparcada, y siempre calcula con TODOS los ojos del caso (mismo alcance que D55, nunca solo el que se esté mirando) | Petición expresa del dueño del proyecto (14/09/2026): «pon la posibilidad de elegir las dos y que haga directamente los cálculos de las dos». Se investigó primero la alternativa de guardar los resultados de las DOS lentes en paralelo (sin intercambiar), y se descartó explícitamente: es la misma decisión que D55 ya tomó y por el mismo motivo —`CONSTANTE_A` es un campo por ojo, no por lente, así que las dos lentes activas a la vez podrían mandarle a Barrett la constante equivocada sin que nadie lo notara—, así que no hacía falta reabrir esa decisión, solo automatizar los pasos manuales que ya existían. Verificado con dos tests unitarios que espían `calcular()`/`generarPdf()`/`intercambiarLentes()` y comprueban el orden exacto de la secuencia (sin montar un navegador real), y un test de interfaz que comprueba que el botón no existe sin lente alternativa aparcada y aparece, con las dos lentes nombradas, en cuanto se aparca una — no llega a pulsarlo, porque encadenaría dos cálculos reales contra EVO/Barrett/Kane y ningún test de `flujo.spec.ts` lanza uno de verdad |
