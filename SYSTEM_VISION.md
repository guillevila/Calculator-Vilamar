...
| D74 | **La constante A del cambio de lente, la auditoría de discrepancia de constante y las 10 reglas clínicas ahora recorren TODOS los aparatos de cada ojo (D47), no solo el principal por defecto.** Tres fallos reales, misma causa: `ojoDe(caso, lado)` sin decir el aparato cae en «Principal» por defecto, y tres funciones escritas antes de D47 nunca se actualizaron para recorrer varios aparatos | Fallo reportado por el dueño del proyecto (14/09/2026) con un caso real de dos aparatos y dos lentes (D55): al cambiar de lente, la constante nueva solo se aplicaba a un aparato «Principal» fantasma —creado sobre la marcha, sin datos reales—, mientras los dos aparatos de verdad se quedaban con la constante de la lente ANTERIOR, sin avisar. Confirmado con los datos reales del caso CV-2026-0143. Al buscar el mismo patrón por el código se encontraron dos fallos más de la misma familia (`discrepanciasDeConstante()`, `ServicioCasos.validar()`), sin ningún test hasta entonces; el dueño pidió arreglar los tres. Ver `docs/CHANGELOG.md` [1.15.31] |
