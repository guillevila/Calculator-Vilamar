...
| D39 | **El PDF final se reduce a: la captura de cada calculadora, una línea con la lente recomendada, y un aviso si alguna no pudo calcular. Nada de tabla comparativa, diagramas, biometría ni trazabilidad** | Petición expresa del dueño del proyecto (25/08/2026), elegida entre tres opciones. Sustituye a D37 al día siguiente: ya no basta con poner las capturas primero, el informe entero se reduce a eso. El informe detallado (portada, comparación, alternativas, biometría, diagramas del ojo, trazabilidad) **no se borra** —viene de una feature ya fusionada a `master`— pero se desconecta del flujo por defecto y queda como código disponible sin usarse (`generarHtmlInformeDetallado`, en `packages/report/src/plantilla.ts`). Ninguna casilla se omite en silencio: una que no tuvo resultado utilizable enseña su propio aviso explicando por qué. Concreta D2 y D13 |
