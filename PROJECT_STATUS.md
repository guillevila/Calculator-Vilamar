...
| **Kane cambia su fórmula o su catálogo sin avisar, y el adaptador NO se entera** | **Alta** | Confirmado el 09/09/2026: Kane (iolformula.com) cambió su cálculo de potencia esférica entre el 27/08 y la semana del 07/09 — probado reproduciendo en vivo un caso guardado que antes coincidía con EVO (22.5D) y hoy da 3-5D menos con los mismos datos exactos; su catálogo de lentes también cambió (un modelo que antes reconocía, ahora no). El adaptador no falla ni avisa por sí solo, pero **desde el 09/09/2026 (mismo día) hay una alarma automática** (petición expresa del dueño: «pon esa alarma»): si el rango entre lo que devuelven las calculadoras es ≥ 2 D, sale una hoja en rojo «Diferencia importante entre calculadoras» en el PDF real y el mismo aviso en el panel de resultados de la app — funciona aunque Kane no destaque ninguna potencia, que es justo el caso real que lo disparó. No dice cuál de las tres está equivocada, solo que hay que mirarlo con más atención. Ver `packages/domain/src/comparacion/comparar.ts` (tipo `'ALARMA'`) y `docs/CHANGELOG.md` [1.15.33]. **Nota (10/09/2026): parte del problema de Kane era en realidad otra cosa** — el botón «Calculate» se pulsaba antes de que el reCAPTCHA invisible de Kane terminara, así que la petición salía sin su token; añadida una espera de 8s antes de calcular (`kane.ts`, `pulsarCalcular`). Ver `docs/CHANGELOG.md` [1.15.35] y la lección del 10/09/2026 en el log.                                                                                                                            |
